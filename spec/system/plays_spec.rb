...
RSpec.describe 'プレー投稿', type: :system do
  ...
  describe "投稿プレー編集" do
    ...
    context '編集できるとき' do
      ...
      it 'ログインしたユーザーは自分が投稿したプレーの編集ができる' do
        ...
        expect(find('#play_area_id').value).to eq "#{@play1.area.id}"
        expect(find('#address').value).to eq(@play1.place)
        expect(find('#play_ground_style_id').value).to eq "#{@play1.ground_style.id}"
        ...
        visit playzones_path
        expect(page).to have_link("#{@play1.title}", href: play_path(@play1))
      end
  ...
  describe "投稿プレー詳細" do
    ...
    it 'ログインしたユーザーはプ詳細ページに遷移してコメント投稿欄が表示される' do
      ...
      expect(page).to have_content("#{@play.area.name}")
      expect(page).to have_content("#{@play.place}")
      expect(page).to have_content("#{@play.ground_style.name}")
      expect(page).to have_content("#{@play.detail}")
      # コメント用のフォームが存在する
